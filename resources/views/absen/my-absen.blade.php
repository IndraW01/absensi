<x-app-layout title="Absen - My Absen">
    <h1 class="h3 mb-4 text-gray-800">My Absen</h1>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">My Absen</h6>
        </div>
        <div class="card-body">
            <form id="formSearch">
                <div class="row mb-2">
    ...
    <script>
        ...
        btnSearch.addEventListener('click', (event) => {
            ...
            axios.post(`{{ route('absen.filterAbsen') }}`, data,
            {
                headers: {
            ...
            .then(response => {
                ...
                $(function () {
                    var table = $('#myAbsenDataTable').DataTable({
                        ...
                        ajax: `{{ url('absen/get-myAbsens') }}?tanggal_awal=${valueTanggalAwal}&tanggal_akhir=${valueTanggalAkhir}`,
                        columns: [
        ...
        $(function () {
            var table = $('#myAbsenDataTable').DataTable({
                ...
                pageLength: 5,
                lengthMenu: [5, 10, 20, 50, 100, 200, 500],
                ajax: "{{ route('absen.myAbsen.datatable') }}",
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'tanggal', name: 'tanggal'},
                    {data: 'status', name: 'status'},
